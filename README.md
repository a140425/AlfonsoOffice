...
# 在 Clawdbot 中說：
請下載專案並完成設定
專案路徑：https://github.com/inventra/Alfonsoffice
到 ~/Desktop/virtual-office
再讀取 ~/Desktop/virtual-office/SETUP.md 並完成設定
```
...
# 選配：Agent 偵測路徑
CLAWDBOT_AGENTS_PATH=~/.openclaw/agents/
```

### Agent 偵測路徑設定

如果在 Docker 環境中使用 Agent 偵測功能，需要 mount 本機的 `.openclaw` 目錄：
...
services:
  app:
    volumes:
      - ~/.openclaw:/root/.openclaw:ro  # 唯讀掛載
```
